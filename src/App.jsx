...
        { icon: <Sparkles size={20} className="text-amber-500"/>, free: 'Limited AI Credits (180/mo)', pro: 'Boosted AI Credits (500/mo)', key: 'credits' },
...
                                <button onClick={onUpgrade} className="w-full py-3 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-lg text-lg transition">Upgrade to Pro</button>
                            </div>
                        </div>
                    </div>
                </div>
...
// --- SIDEBAR COMPONENT ---
const Sidebar = ({ user, folders, decks, activeId, viewMode, onSelectDeck, onSelectFolder, onAddFolder, onDeleteFolder, onRenameFolder, onAddDeck, onDeleteDeck, onSettings, isPro, onShowUpgradeModal }) => {
...
                    <button onClick={onShowUpgradeModal} className="w-full flex items-center justify-center gap-2 p-2.5 bg-amber-500 hover:bg-amber-400 rounded-lg text-sm text-white transition font-bold border border-amber-600"><Award size={16} /> Upgrade to Pro</button>
                </div>
            )}
            <div className="p-4 border-t border-slate-800 shrink-0">
...
                                     <button onClick={() => toggleStudyMode('srs')} disabled={userProfile.subscription?.tier !== 'pro'} className={`px-3 py-1 rounded-md text-xs font-bold transition ${deck.studyMode === 'srs' ? 'bg-white shadow text-indigo-600' : 'text-slate-500'} disabled:opacity-50 disabled:cursor-not-allowed`}>Smart</button>
                                 </div>
                             </div>
...
                                <button onClick={() => setGlobalStudyMode('srs')} disabled={userProfile.subscription?.tier !== 'pro'} className={`flex-1 py-1.5 px-3 rounded-md text-sm font-bold transition ${globalStudyMode === 'srs' ? 'bg-white text-indigo-600 shadow' : 'text-indigo-100 hover:bg-white/10'} disabled:opacity-50 disabled:cursor-not-allowed`}>Smart (SRS)</button>
                            </div>
...
    const addDeck = (fid) => { 
        if (!isProUser && folders.length > 1) return alert("Free plan is limited to one subject (folder). Please upgrade to Pro for unlimited subjects.");
        const nid = Date.now(); 
...
            if (!isProUser && folders.length >= 1) {
                alert("Free plan is limited to one subject (folder). Please upgrade to Pro for unlimited subjects.");
            } else {
...
        alert("Upgrade successful! Welcome to KonDeck Pro.");
    };
...
                isPro={isProUser} // Pass prop to hide button if already pro
                onShowUpgradeModal={() => setShowUpgradeModal(true)} // Handle modal open
            />
...
                                        <button onClick={() => {setShowSettings(false); setShowUpgradeModal(true);}} className="mt-4 w-full bg-gradient-to-r from-amber-500 to-orange-500 text-white font-bold py-2 rounded-lg text-sm">Upgrade to Pro ($33.99/mo)</button>
                                    )}
                                </div>
                            </div>
                            <div>
