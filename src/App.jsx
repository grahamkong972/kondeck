    ...
    for (let attempt = 0; attempt < 3; attempt++) {
        try {
            ...
            try {
                ...
                // Recovery mechanism for truncated JSON
                ...
                throw e; // Re-throw if repair fails
            }

        } catch (error) {
            console.warn(`Attempt ${attempt + 1} failed:`, error.message);
            if (attempt === 2) throw error; // Fail after 3 attempts
            await sleep(2000 * (attempt + 1)); // Wait 2s, then 4s
        }
    }
};
...
const Dashboard = ({ deck, onUpdateDeck, apiKey }) => {
    ...
    // State for the three input types
    ...
    const handleGenerate = async () => {
        ...
        // Save current input state before generating
        ...
        try {
            ...
            for (let i = 0; i < totalBatches; i++) {
                ...
                // Rate Limiting Pause: Wait 1s between batches to avoid 429 errors
                ...
                if (genType === "flashcards") {
                    prompt = `Generate ${currentBatchCount} unique, high-quality flashcards from the context.${avoidInstruction}`;
                } else {
                    prompt = `Generate ${currentBatchCount} unique multiple choice questions from the context.${avoidInstruction}`;
                }
                ...
                try {
                    ...
                    alert(`Generation stopped early at batch ${i+1} due to an AI error. Saving the ${accumulatedResults.length} items generated so far.`);
                    break; // Stop loop but proceed to save what we have
                }
            }
